...
// Related Topics Array Two Pointers Sorting 👍 1242 👎 143
...
class MSASolution {
    public void merge(int[] nums1, int m, int[] nums2, int n) {
        ...
        while (firstIdx >= 0 && secondIdx >= 0) {
            if (nums1[firstIdx] >= nums2[secondIdx]) {
                nums1[idx] = nums1[firstIdx];
                firstIdx--;
            } else {
                nums1[idx] = nums2[secondIdx];
                secondIdx--;
            }
            idx--;
        }
    ...
    public static void main(String[] args) {
        int[] nums1 = {2,0};
        int[] nums2 = {1};
        MSASolution msaSolution = new MSASolution();
        msaSolution.merge(nums1, 1, nums2, 1);
    }
}
